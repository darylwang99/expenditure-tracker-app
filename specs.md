...
- **CSV export**: exports the currently filtered/sorted/searched list (columns: Date, Category, Payment Method, Amount, Note, Has Receipt). Receipt images are never embedded in the CSV. Any field starting with `=`, `+`, `-` or `@` (e.g. in a note) is prefixed with `'` to prevent it being interpreted as a formula by Excel/Google Sheets when opened (CSV/formula-injection protection).
- **Full JSON backup/restore**: "Backup" downloads everything (expenses, categories, budgets, currency, recurring templates) as one JSON file; "Restore" replaces current data from such a file after confirmation, and is itself undoable via the toast. A restore file over `BACKUP_MAX_BYTES` (20MB) is rejected before being read/parsed, the same size-guard pattern used for receipts and statement PDFs.
...
- `index.html` has a strict Content-Security-Policy (`script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'none'; worker-src 'self'`), duplicated as a real HTTP response header in `vercel.json` (with `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Strict-Transport-Security`, and `Referrer-Policy`) for the deployed site. The `<meta>` tag alone is kept too, since it's the only CSP enforcement available when the app is opened directly via `file://`, but a `<meta>` CSP cannot enforce `frame-ancestors` at all (browsers silently ignore it there) — the `vercel.json` header is what actually provides clickjacking protection on the live site. No inline `<script>`, inline `<style>`, `style=""` or `onclick=""` attributes, and no external resources (fonts, CDNs, chart libraries) — `pdf.min.js`/`pdf.worker.min.js` are vendored (downloaded once and committed) rather than loaded from a CDN. All charts are hand-drawn inline SVG.
