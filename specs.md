...
- `index.html` has a strict Content-Security-Policy (`script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'none'`). No inline `<script>`, inline `<style>`, `style=""` or `onclick=""` attributes, and no external resources (fonts, CDNs, chart libraries). All charts are hand-drawn inline SVG.
